...
user_results <- lapply(user_id_vec, function(userID){
  user_labels <- user_knot_labels %>% 
                    ...
                    group_by(knotname) %>% 
                    filter(labeltime==min(labeltime)) %>% # only keep the last label they chose
                    as.data.frame
...
Note that `r 100 * sum(user_results_matrix[,"accuracy"] > official_results['accuracy'])/nrow(user_results_matrix)` percent of the participant labels led to models with better accuracy than the "official" labels. Part of the reason for this apears to be that the official images include many that are very low resolution. since the workshop last week, I have removed the lowest resolution images, and the performance of the actively learned model has improved. Still, even with the 
